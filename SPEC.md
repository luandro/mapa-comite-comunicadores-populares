...
- Org label boxes use dark brown (`#513822`) with cream (`#fdf6e3`) text, centered below each totem base, with compact multiline wrapping. City display names come from `data.json`: Belém, Ananindeua and Moju (an empty projects map still renders its city label). Totems are 190 scene units tall, anchored at their base.
