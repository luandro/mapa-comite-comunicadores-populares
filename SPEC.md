...
**City tap (issue #12 rework — replaces the v1.0 lift/dim):** NO lift, NO dim, NO outline rectangle. Tapping a city re-runs the entrance choreography for ONLY the organizations connected to that city (an org belongs to the city whose `maps.<city>.projects` carries its id): their totems drop in again (`ARTIFACT_DROP_FROM`) and their arrows dash-draw with the tail-dot fade — orgs replay strictly one at a time (the effective stagger retires one org's whole tween family before the next starts, holding the SPEC §11 ≤4-concurrent-tweens budget; a 7-org city takes ~3.5 s), as if the app freshly rendered for that city. `focusCity` still flies the camera. Tap empty (or the selected city again) = re-finalize the replayed orgs to the pristine post-intro rest state — nothing lingers.
...
- Tap city → camera flies and ONLY that city's connected orgs replay their entrance (totem drop + arrow dash-draw with tail-dot fade; no lift, no dim — issue #12); tap empty resets everything to the pristine post-intro rest state.
