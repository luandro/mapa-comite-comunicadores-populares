...
/**
 * Uniform width-fit (SPEC §5 / AGENTS 5): waves AND the squiggle texture are
 * 2160.32-wide assets (SPEC §1); their placement groups mount
...
export type TargetKind = 'underlay' | 'city' | 'wave' | 'squiggle'
...
export interface CalibTarget {
  ...
  layerId: string | null
  /** fallback index among the layer's outermost placement groups (waves: band order) */
  index: number
  placement: MutablePlacement
}

type CityId = 'belem' | 'ananindeua' | 'moju'
type WaveId = 'onda1' | 'onda2' | 'onda4'
...
export function buildTargets(): CalibTarget[] {
  ...
  const waves = (Object.keys(WAVE_LABELS) as WaveId[]).map((id, index): CalibTarget => ({
    id,
    kind: 'wave',
    label: WAVE_LABELS[id],
    layerId: 'layer-waves',
    index,
    placement: wavePlacements[id] ? { ...wavePlacements[id] } : { ...DEFAULT_PLACEMENT },
  }))
  return [
    {
      id: 'underlay',
...
export function placementTransform(target: CalibTarget): string {
  ...
  const composesFit = target.kind === 'wave' || target.kind === 'squiggle'
...
export function placementSnippet(target: CalibTarget): string {
  ...
  body += ' }'
  // Record members paste straight into cityPlacements/wavePlacements; the
  // single-record exports (squiggle/underlay) take the bare object literal.
  return target.kind === 'city' || target.kind === 'wave' ? `'${target.id}': ${body},` : body
}
 ...
 * Wave bands carry `data-band="ondaN"` (index is the DOM-order fallback,
 * SPEC §4: onda 1/2/4).
 */
export function findPlacementGroup(
  ...
  if (target.kind === 'wave') {
    const byBand = layer.querySelector(`:scope > g[data-band="${target.id}"]`)
    if (byBand instanceof SVGGElement) return byBand
  }
