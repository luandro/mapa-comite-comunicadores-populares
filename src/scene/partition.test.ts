...
describe('partitionBase', () => {
  it('splits mapa cru into the SPEC §2 layers, preserving document order', () => {
    ...
    expect([layers.land.length, layers.roads.length, layers.waterDetail.length]).toEqual([
      42, 5, 20,
    ])
  ...
  it('ignores the sea rect — v1.1 cut the ocean (CSS background carries it)', () => {
    const layers = partitionBase(processedMapaCru())
    const sea = layers.land.concat(layers.roads, layers.waterDetail)
    expect(sea).toHaveLength(67) // 42 + 5 + 20 — the #5da9a9 rect is not mounted
    for (const node of sea) {
      expect(node.getAttribute('fill')?.toLowerCase()).not.toBe('#5da9a9')
    }
  })

  it('throws on a <g> wrapper — reparenting would drop ancestor transforms', () => {
    const wrapped =
      '<svg xmlns="http://www.w3.org/2000/svg"><g><path fill="#95c98e" d="M0 0"/></g></svg>'
    expect(() => partitionBase(wrapped)).toThrow(/not flat: <g>/)
  })

  it('throws on a painted node carrying a transform attribute', () => {
    const shifted =
      '<svg xmlns="http://www.w3.org/2000/svg"><path fill="#95c98e" ' +
